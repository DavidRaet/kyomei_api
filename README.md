...
`kyomei_api` is a FastAPI backend-for-frontend (BFF) for [`kyomei_0`](https://github.com/DavidRaet/kyomei_0)<!-- adjust link if the frontend repo path differs -->, a TypeScript/Vite anime-tracking client. It orchestrates anime metadata from two upstream sources — [AniList](https://anilist.co) (GraphQL, primary) and [Jikan](https://jikan.moe) (REST, fallback) — behind a single cached HTTP API, and is intended to eventually add personalized recommendation logic on top (see `docs/Kyomei-MVP-PRD-v2.1.md`).
...
**Not yet done:**
- Core domain logic — `app/anime/`, `app/anilist/`, `app/jikan/`, `app/cache/`, `app/routers/`, and `app/config.py` are all empty stub modules (a one-line comment describing intended contents, no implementation)
...
`kyomei_api` is a BFF, not the recommendation engine described in the PRD — its job in this phase is orchestration and caching only. It mirrors, server-side, the fallback pattern the frontend already implements client-side (`kyomei_0`'s `src/api/animeProvider.ts` → `anilist.ts` primary / `jikan.ts` fallback / `cache.ts`).
...
```
  ...
  (frontend)     │   (BFF)    │
            ...
            try AniList (GraphQL)
                       │  on error/timeout
                       ▼
              fall back to Jikan (REST)
```

Request flow per endpoint: check cache → query AniList → on error/timeout, query Jikan → cache the result → return a normalized `AnimeSummary`/`CharacterSummary` shape (see `CONTRACT.md`) regardless of which upstream source served it.
...
| Outbound HTTP client | `httpx` (≥0.28) — planned for AniList/Jikan clients | `pyproject.toml` |
...
│   ├── anime/        # stub — domain logic: Provider Protocol/ABC, AniList→Jikan fallback orchestration
│   ├── anilist/      # stub — AniList GraphQL client (httpx)
│   ├── jikan/        # stub — Jikan REST client (httpx), fallback only
...
│   └── config.py     # stub — pydantic-settings env/config loading (PORT, ANILIST_ENDPOINT, JIKAN_BASE_URL, CACHE_TTL_SECONDS)
├── tests/
│   └── test_health.py
├── docs/
...
|---|---|---|---|
| [AniList](https://docs.anilist.co/guide/graphql/) | Primary | GraphQL | `https://graphql.anilist.co` |
| [Jikan](https://docs.api.jikan.moe/) | Fallback (on error/timeout) | REST | `https://api.jikan.moe/v4` |
...
| `JIKAN_BASE_URL` | `https://api.jikan.moe/v4` | Jikan REST base URL |
...
Current coverage: `tests/test_health.py` (a `TestClient` request against `GET /health`). Per the setup checklist's Testing section, still open: unit tests for the AniList/Jikan clients (mocked HTTP), unit tests for fallback orchestration logic, and an integration test against a running server.
...
- Implement core domain logic: `app/anime/` orchestration, `app/anilist/` and `app/jikan/` clients, `app/cache/`, `app/routers/`, `app/config.py`
...
<!-- HUMAN INPUT: Credit anyone/anything you'd like to acknowledge (e.g. AniList and Jikan as data providers, prior art, people who helped). -->
